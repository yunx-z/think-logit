...
def generate_completions(
    ...
    **generation_kwargs
    ...
    for i in range(0, len(prompts), batch_size):
        ...
        if output_file:
            predictions = [my_answer_extraction(output, problem_type=problem_type) for output in batch_generations]
...
def load_dexperts_model_and_tokenizer(
    ...
    # tokenizer = add_pad_token(tokenizer, padding_side)
    base_tokenizer = AutoTokenizer.from_pretrained(base_model_name_or_path, use_fast_tokenizer=False)
    base_tokenizer = add_pad_token(base_tokenizer, padding_side)
    if expert_model_name_or_path is None:
        expert_tokenizer = None
    else:
        expert_tokenizer = AutoTokenizer.from_pretrained(expert_model_name_or_path, use_fast_tokenizer=False)
        expert_tokenizer = add_pad_token(expert_tokenizer, padding_side)
    ...
    model = DExpertsLlama(
        ...
        expert_tokenizer=expert_tokenizer,
