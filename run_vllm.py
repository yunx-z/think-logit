...
def run_vllm_concurrent(prompts, model, temperature):
    llm = LLM(
            model=model, 
    ...
    sampling_params = SamplingParams(temperature=temperature, max_tokens=8192)
...
def main(args):
    ...
    if len(test_data) == 0:
        print("No data to process, skipping generation.")
        return
    ...
    temperature = 0.6 if args.do_sample else 0.0
    outputs = run_vllm_concurrent(prompts, args.model_name_or_path, temperature)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str_to_bool,
        default=False,
        help="If True, use temperature 0.6 for sampling, else 0."
    )
    parser.add_argument(
        "--budget_forcing",
        type=str_to_bool,
